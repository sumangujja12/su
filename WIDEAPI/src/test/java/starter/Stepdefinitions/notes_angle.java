...
public class notes_angle {
	
	@When("Notes for Angle Post Rerquest")
	public void vessel() throws FileNotFoundException{
		...
		File f= new File("payload_notes_angle.json");
		...
		Response response=request.body(ob.toString()).header("Content-Type","application/json").header("Authorization",accesstoken).post("/api/v2.1/notes");
